...
const BooksDetailsScreen = () => {
    ...
    const { data: result, isSuccess: searchIsSuccess } = useGetBookByTitleQuery(searchTerm)
    ...
    return (
        <div className='px-40 py-20 w-full h-max '>
            <div className='mx-16 my-10'>
                <SearchInputComponent value={searchTerm} onChange={(e) => { setSearchTerm(e.target.value) }} onClick={() => { console.log('serchterm :', searchTerm) }} />
            </div>
            <div className='grid gap-4 grid-cols-3 grid-rows-3 w-full flex ml-16 '>
                {/* {isSuccess && books["hydra:member"]?.map((book: any) => {
                    return (
                        <BookCardComponent onClick={() => navigate(`/book/${book.id}`)} key={book.id} book={book} />
                    )
                })
                } */}
                {searchIsSuccess ? (
